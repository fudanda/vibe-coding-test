...
- Token 消耗：
- 提交与 PR 准备流程：未使用 | 已使用
- AI Review：已自动执行 Code Reviewer | 跳过，原因： | 超时/不可用，原因：
- AI Review 快照：
- AI Review 线程：
- AI Review 是否过期：
- Git 提交方式：独立提交准备对话自动本地提交 | 当前对话直接提交（用户已明确要求） | 人工提交 | 未提交，原因：
- 本地 Commit：
- 人工确认过的关键逻辑：
- 未采用的人工智能建议：
...
- [ ] Review 字段已填写；默认已记录 `AI：Codex/code-reviewer`，如跳过已写明原因
...
- [ ] Code Reviewer 已自动执行；如跳过，原因已记录。
- [ ] 本地 commit 已在独立提交准备对话中按自动提交门禁执行；如在当前对话直接提交，已记录用户明确授权语；如未提交，阻塞原因已记录。
- [ ] `push` 和 PR 创建已单独确认。
- [ ] 没有使用 `git add .`。
