...
团队希望每次功能修改完成后，提交和 PR 准备不继续占用开发对话，而是在新的独立 Codex 对话中按档位 2 执行，降低开发、Review 和 git 操作上下文混用的风险。

## 改了什么

- 在本项目 `AGENTS.md` 的“提交和 PR 准备自动化”中加入独立档位 2 对话规则。
- 明确当前开发对话只输出交付交接包。
...
- 已运行：`rg "独立 Codex 对话|独立档位 2|交付交接包|交接包|档位 2" ...`，确认 `vibe-coding-test/AGENTS.md` 已包含独立档位 2 对话规则。
...
- 真实 `git add`、`commit`、`push` 和 PR 创建仍必须等待人工确认。
